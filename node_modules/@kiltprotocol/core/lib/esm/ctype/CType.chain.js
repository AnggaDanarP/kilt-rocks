...
import { getHashForSchema, hashToId, idToHash, serializeForHash, verifyDataStructure, } from './CType.js';
...
function cTypeInputFromChain(input) {
    try {
        ...
        throw new SDKErrors.CTypeError(`The provided payload cannot be parsed as a CType: ${input.toHuman()}`, { cause });
    }
}
 ...
 */
export async function fetchFromChain(cTypeId) {
    ...
    if (typeof createdAt === 'undefined')
        throw new SDKErrors.CTypeError('Cannot fetch CType definitions on a chain that does not store the createdAt block');
    const extrinsic = await retrieveExtrinsicFromBlock(api, createdAt, ({ events }) => events.some((event) => api.events.ctype.CTypeCreated.is(event) &&
        event.data[1].toString() === cTypeHash));
    if (extrinsic === null) {
        throw new SDKErrors.CTypeError(`There is not CType with the provided ID "${cTypeId}" on chain.`);
    }
    if (!isBatch(api, extrinsic) &&
        !api.tx.did.submitDidCall.is(extrinsic) &&
        !api.tx.did.dispatchAs.is(extrinsic)) {
        throw new SDKErrors.PublicCredentialError('Extrinsic should be either a `did.submitDidCall` or `did.dispatchAs` extrinsic or a batch containing at least one of those');
    }
    ...
    if (!lastRightCTypeCreationCall) {
        throw new SDKErrors.CTypeError('Block should always contain the full CType, eventually.');
    }
