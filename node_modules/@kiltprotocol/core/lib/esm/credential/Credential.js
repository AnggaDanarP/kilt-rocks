 ...
 */
export function verifyWellFormed(credential, { ctype } = {}) {
    verifyDataStructure(credential);
    verifyDataIntegrity(credential);
    if (ctype) {
        if (`kilt:ctype:${credential.claim.cTypeHash}` !== ctype.$id) {
            throw new SDKErrors.CTypeIdMismatchError(ctype.$id, credential.claim.cTypeHash);
        }
 ...
 */
export async function verifyAttested(credential) {
 ...
 */
export async function refreshRevocationStatus(verifiedCredential) {
    ...
    if (attester !== verifiedCredential.attester) {
        throw new SDKErrors.CredentialUnverifiableError('Attester has changed since first verification');
    }
