 ...
 */
export function getIdForCredential(credential, attester) {
    const api = ConfigService.get('api');
    const scaleEncodedCredential = api
        .createType('PublicCredentialsCredentialsCredential', publicCredentialToChain(credential))
        .toU8a();
    const scaleEncodedAttester = api
        .createType('AccountId', Did.toChain(attester))
        .toU8a();
    return blake2AsHex(Uint8Array.from([...scaleEncodedCredential, ...scaleEncodedAttester]));
}
...
export async function verifyCredential(credential, { cType } = {}) {
    ...
    if (encodedCredentialEntry.isNone) {
        throw new SDKErrors.PublicCredentialError('Provided credential does not exist.');
    }
    ...
    if (!blockNumber.eq(retrievedBlockNumber)) {
        throw new SDKErrors.PublicCredentialError(`Block number in the credential (${credential.blockNumber}) different than what is stored on the blockchain (${retrievedBlockNumber.toBn()}).`);
    }
    if (revoked !== retrievedRevocationStatus.toPrimitive()) {
        throw new SDKErrors.PublicCredentialError(`Revocation status in the credential (${credential.revoked}) different than what is stored on the blockchain (${retrievedRevocationStatus.toPrimitive()}).`);
    }
}
/**
 ...
 */
export function fromClaim(claim, { delegationId = null } = {}) {
    const credential = {
