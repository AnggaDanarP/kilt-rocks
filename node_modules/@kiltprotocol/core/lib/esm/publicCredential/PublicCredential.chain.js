 ...
 */
export function toChain(publicCredential) {
 ...
 */
export async function fetchCredentialFromChain(credentialId) {
    ...
    const extrinsic = await retrieveExtrinsicFromBlock(api, blockNumber, ({ events }) => events.some((event) => api.events.publicCredentials.CredentialStored.is(event) &&
        event.data[1].toString() === credentialId));
    if (extrinsic === null) {
        throw new SDKErrors.PublicCredentialError(`The block number as specified in the provided credential entry (${blockNumber}) does not have any extrinsic that includes a credential creation.`);
    }
    ...
    const callCredentialsContent = didCalls.flatMap((didCall) => {
        ...
        return publicCredentialCalls.map((credentialCreationCall) => [
            ...
            didFromChain(api.tx.did.submitDidCall.is(didCall)
                ? didCall.args[0].did
                : didCall.args[0]),
        ]);
    });
    ...
    if (!lastRightCredentialCreationCall) {
        throw new SDKErrors.PublicCredentialError('Block should always contain the full credential, eventually.');
    }
 ...
 */
export async function fetchCredentialsFromChain(subject) {
    const api = ConfigService.get('api');
