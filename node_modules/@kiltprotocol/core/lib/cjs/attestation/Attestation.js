...
function verifyAgainstCredential(attestation, credential) {
    const credentialMismatch = credential.claim.cTypeHash !== attestation.cTypeHash;
    const ctypeMismatch = credential.rootHash !== attestation.claimHash;
    const delegationMismatch = credential.delegationId !== attestation.delegationId;
    if (credentialMismatch || ctypeMismatch || delegationMismatch) {
        throw new utils_1.SDKErrors.CredentialUnverifiableError(`Some attributes of the on-chain attestation diverge from the credential: ${[
            'cTypeHash',
            'delegationId',
            'claimHash',
        ]
            .filter((_, i) => [ctypeMismatch, delegationMismatch, credentialMismatch][i])
            .join(', ')}`);
    }
