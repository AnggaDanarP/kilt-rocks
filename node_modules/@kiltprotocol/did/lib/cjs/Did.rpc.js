...
exports.linkedInfoFromChain = void 0;
...
function serviceFromChain(encoded) {
    ...
    return {
        id: `#${(0, util_1.u8aToString)(id)}`,
        type: serviceTypes.map(util_1.u8aToString),
        serviceEndpoint: urls.map(util_1.u8aToString),
    };
}
...
function accountFromChain(account, networkPrefix = utils_1.ss58Format) {
    if (isLinkableAccountId(account)) {
        ...
        if (account.isAccountId32)
            return (0, keyring_1.encodeAddress)(account.asAccountId32, networkPrefix);
        // linked account is ethereum address (ethereum-enabled storage version)
        if (account.isAccountId20)
            return (0, util_crypto_1.ethereumEncode)(account.asAccountId20);
    }
    // linked account is substrate account (legacy storage version)
    return (0, keyring_1.encodeAddress)(account.toU8a(), networkPrefix);
}
function connectedAccountsFromChain(encoded, networkPrefix = utils_1.ss58Format) {
 ...
 */
function linkedInfoFromChain(encoded, networkPrefix = utils_1.ss58Format) {
...
exports.linkedInfoFromChain = linkedInfoFromChain;
